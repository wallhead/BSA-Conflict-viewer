...
class BsaConflictDialog(QDialog):
    def __init__(self, plugin: "BsaConflictViewerPlugin", parent=None):
        ...
        self._mod_sort.addItem("Cleanup space", "cleanup")
    ...
    def export_csv(self) -> None:
        ...
        try:
            with open(filename, "w", newline="", encoding="utf-8-sig") as handle:
                ...
                selected_bytes = self._reclaimable_by_mod.get(summary.mod_name, 0)
                if archive is not None:
                    selection_name = f"{summary.display_mod_name} :: {archive.archive_name}"
                    selected_bytes = self._reclaimable_bytes_for_archive(
                        archive, self._enabled_categories(), self._filter_text(self._file_filter)
                    )
                writer.writerow([selection_name])
                writer.writerow(["selected_reclaimable_file_bytes", selected_bytes])
                writer.writerow(["total_reclaimable_file_bytes", self._total_reclaimable_bytes])
                writer.writerow([])
